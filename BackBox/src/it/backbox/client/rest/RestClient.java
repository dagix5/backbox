...
public class RestClient {
	...
	public RestClient() throws Exception {
		final Credential credential = OAuth2Client.getCredential();
	...
	public byte[] download(String fileID) throws IOException, NumberFormatException, InterruptedException {
		...
		OutputStreamCounter outputStream = new OutputStreamCounter(baos, "");
	...
	public BoxFile upload(String name, String fileID, byte[] content, String folderID, String sha1) throws RestException, IOException {
		...
		mpc.addPart(new MultipartFormDataContent.Part(null, name, new InputStreamContent("application/octet-stream", new InputStreamCounter(new ByteArrayInputStream(content), ""))));
...
//				ids.add((String) ((JSONObject) entry).get("id"));
//			}
//			return ids;
//		} catch (ParseException e) {
//			throw new RestException(e.getLocalizedMessage());
